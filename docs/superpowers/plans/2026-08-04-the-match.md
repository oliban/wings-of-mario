...
- `mario-overlay.js` — `OVERLAY_ID`, `MAX_CATCHUP`, `class MarioOverlay` (`attach(api)`, `pump()`, `step()`, `draw()`, `add(release)`, `sync`, `reset`, `hooks`, `marks`, `telegraph`, `whistle`, `synth`). `add()`/`sync()` call `geo.js`'s `assertLocalY` on the incoming y — see Step 5a.
...
- [ ] **Step 5a: Guard the coordinate seam `add()`/`sync()` sit on**

`add()` and `sync()` are the seam the Global Constraints section names: "Conversion happens once, at the seam, in `mario-overlay.js`." Right now the only caller is `window.__TELEGRAPH.drop()`, which builds island-local coordinates by hand and cannot get this wrong. Task 8 changes that: its `net-bridge.js` converts the pilot's world-space release into island-local pixels and hands the result straight to these two methods. **If a future edit ever skips that conversion — passes `release.y` straight through instead of `toLocal(originX, release.x, release.y).y`, say — the bug is invisible by construction**: the arithmetic still runs, and the reticle simply appears `ISLAND_TOP_Y` (320px) below where it should, off the bottom of a 240px-tall screen. Nothing throws, nothing looks wrong in a diff, and the manual test in Step 10 would just show no reticle at all.

`src/wings/geo.js` now exports `assertLocalY(y, label)` for exactly this. It is one-directional on purpose: an island-local level is only `ISLAND_H` (240) px tall, so a local y can never legitimately reach `ISLAND_TOP_Y` (320) — that band is reachable only by a world-space y. (The reverse check does not exist and must not be added here: a world-space y can legitimately sit anywhere from `CEILING_Y` up through the local-looking 0..240 range — a plane at altitude has a small world y — so asserting "this world value looks local" would false-positive on ordinary high-altitude flight.) In Node/tests it throws immediately; in the browser it `console.error`s once per label and keeps rendering, because a thrown error mid-frame would blank the game, which is worse than a misplaced reticle for the frames it takes to notice the console message.

Add the import and the two calls to `src/wings/mario-overlay.js`:
...
Add one test to `tests/unit/mario-overlay.test.js` proving it (add `ISLAND_TOP_Y` to that file's import from `../../src/wings/geo.js`):
...
Run: `npm run test:unit`. Expected: PASS, two tests added.
...
Run: `npm run test:unit` — Expected: PASS, 202 — the 192 above plus 8 overlay tests plus the 2 seam-guard tests from Step 5a.

Run: `npm run test:browser` — Expected: PASS — the 57 existing plus this file's 10 subtests (9 plus the on-screen acceptance check from Step 5a).
